...
import { AtomicPopoverCdk } from './atomic-popover-cdk';
import { Trigger } from '../atomic-popover/atomic-popover-utils';

@Component({
  imports: [AtomicPopoverCdk],
  template: `
    <app-atomic-popover-cdk [trigger]="trigger">
      <button trigger>Open</button>
      <div>Popover body</div>
    </app-atomic-popover-cdk>
  `,
})
class HostComponent {
...
const setup = async (trigger: Trigger | Trigger[] = 'click') => {
  ...
  const popover = fixture.debugElement.query(By.directive(AtomicPopoverCdk))
    .componentInstance as AtomicPopoverCdk;
  const reference = fixture.nativeElement.querySelector(
    '.atomic-popover-cdk__reference'
  ) as HTMLElement;

  // popover 內容被 CDK portal 到 body 下的 .cdk-overlay-container,不在 fixture 內。
  const popoverEl = () =>
    document.querySelector('.cdk-overlay-container .atomic-popover-cdk') as HTMLElement | null;

  return { fixture, popover, reference, popoverEl };
};

describe('AtomicPopoverCdk', () => {
  afterEach(() => {
    // overlay container 附掛在 body 上、跨測試存活,每個測試結束後清乾淨。
  ...
  describe('click outside(CDK overlayOutsideClick)', () => {
    it('點擊外部關閉', async () => {
      const { fixture, popover, reference } = await setup('click');
      reference.click();
      fixture.detectChanges();
